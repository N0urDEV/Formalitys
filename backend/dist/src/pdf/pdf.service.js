...
let PdfService = class PdfService {
    async generateCompanyDossierPdf(dossierData) {
    ...
    async generateTourismDossierPdf(dossierData) {
        const browser = await puppeteer.launch({
            headless: true,
            args: ['--no-sandbox', '--disable-setuid-sandbox']
        });
    ...
    generateCompanyDossierHtml(dossierData) {
    ...
    generateTourismDossierHtml(dossierData) {
        const { user, dossier } = dossierData;
